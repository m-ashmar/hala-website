...
/* ═══════════════════════════════════════════════════
   LAZY LOADED — Instagram feed (non-critical, no SSR)
   ═══════════════════════════════════════════════════ */
const InstagramFeedSection = dynamic(
  () => import("./InstagramFeedSection"),
  { ssr: false, loading: () => <InstagramSkeleton /> }
);
...
/* ═══════════════════════════════════════════════════
   INSTAGRAM SKELETON — shown while lazy section loads
   ═══════════════════════════════════════════════════ */
    ...
    <section id="instagram" style={{ padding: "110px 24px", background: "var(--bg-secondary)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 52 }}>
          <span className="skeleton" style={{ display: "inline-block", width: 120, height: 14, borderRadius: 4, marginBottom: 16 }} />
          <div className="skeleton" style={{ width: 280, height: 36, borderRadius: 8, margin: "0 auto" }} />
        </div>
...
const WHY_US_PILLARS = [
  { icon: "✦", titleEn: "Handcrafted Quality", titleAr: "جودة يدوية", descEn: "Every piece is crafted with meticulous care, ensuring unmatched quality in every stitch and detail.", descAr: "كل قطعة تُصنع بعناية فائقة وحرفية عالية." },
  { icon: "◈", titleEn: "Custom Made Orders", titleAr: "طلبات مخصصة", descEn: "Your vision, our craft. We bring your unique style ideas to life with personalized creations.", descAr: "رؤيتك وفنّنا — نحوّل أفكارك إلى إبداع فريد." },
  { icon: "◇", titleEn: "Premium Materials", titleAr: "مواد فاخرة", descEn: "Only the finest fabrics and premium-grade plexi materials make it into our collections.", descAr: "أجود الأقمشة والمواد البليكسية في كل مجموعة." },
  { icon: "◉", titleEn: "Authentic Designs", titleAr: "تصاميم أصيلة", descEn: "Original designs inspired by Arabic heritage, modern elegance, and timeless femininity.", descAr: "تصاميم أصلية مستوحاة من التراث العربي والأناقة العصرية." },
];

/* ═══════════════════════════════════════════════════
   TYPES
   ═══════════════════════════════════════════════════ */
type DbProduct = { id: string; sanityId: string; price: number; stock: number };

export interface HomeClientProps {
  banners: SanityHomepageBanner[];
...
export default function HomeClient({ banners, promotions, sanityProducts, dbProducts, testimonials, heroStats: rawHeroStats }: HomeClientProps) {
  ...
  const heroStats = rawHeroStats.length > 0 ? rawHeroStats : DEFAULT_HERO_STATS;
  const addItem = useCartStore((s) => s.addItem);
  const isRtl = lang === "ar";
  const T = useCallback((key: Parameters<typeof tr>[0]) => tr(key, lang), [lang]);

  useEffect(() => { setLang(params?.locale === "ar" ? "ar" : "en"); }, [params?.locale]);
  ...
  const testimonialData = testimonials.length > 0
    ? testimonials.map((t) => ({ text: isRtl && t.quoteAr ? t.quoteAr : t.quote, author: isRtl && t.authorAr ? t.authorAr : t.author, product: "", rating: t.rating, avatarUrl: t.avatarImageUrl }))
    : [
  ...
  useScrollAnimation([sanityProducts, testimonials]);
  ...
  const showToast = useCallback((msg: string, type: "success" | "error") => { setToast({ msg, type }); setTimeout(() => setToast(null), 5000); }, []);

  const handleAddToCart = useCallback((p: SanityProduct, dbProduct: DbProduct | undefined) => {
    const productName = isRtl && p.titleAr ? p.titleAr : p.title;
    addItem({ productSyncId: dbProduct ? dbProduct.id : `temp-${p.sanityId}`, sanityId: p.sanityId, name: productName, price: dbProduct ? dbProduct.price : p.price });
  ...
  const handleCustomOrderSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (status !== "authenticated") {
      showToast(isRtl ? "يجب تسجيل الدخول أولاً لتقديم طلب مخصص." : "You must be logged in to submit a custom request.", "error");
      router.push(`/${lang}/login?callbackUrl=/${lang}#custom-orders`);
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        requestedQuantity: parseInt(fd.get("requestedQuantity") as string, 10) || 1,
      ...
      res.ok ? (showToast(T("formSuccess"), "success"), (e.target as HTMLFormElement).reset()) : showToast(data.error || "Something went wrong.", "error");
    } catch (err: any) { showToast(err.message || "Network error. Please try again.", "error"); }
    finally { setCustomOrderLoading(false); }
  };
  ...
  const handleContactSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("/api/contact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: fd.get("name"), email: fd.get("email"), message: fd.get("message") }) });
      const data = await res.json();
      res.ok ? (showToast(T("contactSuccess"), "success"), (e.target as HTMLFormElement).reset()) : showToast(data.error || "Something went wrong.", "error");
    ...
    <>
      ...
      <section id="hero" style={{ position: "relative", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
        {banners.length > 0
          ? banners.map((banner, idx) => (
              <div key={banner._id} style={{ position: "absolute", inset: 0, opacity: idx === currentBannerIndex ? 1 : 0, transition: "opacity 1.2s ease-in-out", zIndex: idx === currentBannerIndex ? 0 : -1 }}>
                <Image unoptimized src={banner.backgroundImageUrl} alt={banner.title || "Banner"} fill priority={idx === 0} style={{ objectFit: "cover" }} sizes="100vw" className="max-md:hidden" />
                <Image unoptimized src={banner.mobileImageUrl || banner.backgroundImageUrl} alt={banner.title || "Banner"} fill priority={idx === 0} style={{ objectFit: "cover" }} sizes="100vw" className="md:hidden" />
              </div>
            ))
          : <div style={{ position: "absolute", inset: 0 }}><Image src="/hero-bg.png" alt="Halahello" fill priority style={{ objectFit: "cover" }} sizes="100vw" /></div>
        }
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 1, pointerEvents: "none", overflow: "hidden" }} aria-hidden="true">
          {[...Array(8)].map((_, i) => (
            <div key={i} style={{ position: "absolute", width: i % 3 === 0 ? 6 : i % 2 === 0 ? 4 : 3, height: i % 3 === 0 ? 6 : i % 2 === 0 ? 4 : 3, borderRadius: "50%", background: "rgba(207,161,141,0.6)", left: `${10 + i * 11}%`, top: `${20 + (i % 4) * 18}%`, animation: `particleDrift ${5 + i * 1.5}s ease-in-out ${i * 0.8}s infinite` }} />
          ))}
        </div>
        <div style={{ position: "relative", zIndex: 2, textAlign: "center", maxWidth: 860, padding: "0 24px", width: "100%" }}>
          {banners.length > 0
            ? banners.map((banner, idx) => (
                <div key={banner._id} style={{ display: idx === currentBannerIndex ? "block" : "none" }}>
                  <span className="section-tag" style={{ color: "var(--accent-light)", marginBottom: 20, display: "block" }}>{T("heroTag")}</span>
                  <h1 style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "clamp(2.8rem, 8vw, 6.5rem)", fontWeight: isRtl ? 700 : 600, fontStyle: isRtl ? "normal" : "italic", color: "#FFF", marginBottom: 20, lineHeight: 1.02, letterSpacing: isRtl ? "-0.01em" : "-0.025em", animation: "revealUp 0.9s cubic-bezier(0.22,1,0.36,1) 0.1s both", textShadow: "0 2px 40px rgba(0,0,0,0.4)" }}>
                    {isRtl && banner.titleAr ? banner.titleAr : banner.title}
                  </h1>
                  {(banner.subtitle || banner.subtitleAr) && (
                    <p style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "clamp(1rem,2.5vw,1.4rem)", fontWeight: 400, fontStyle: isRtl ? "normal" : "italic", color: "rgba(255,255,255,0.80)", marginBottom: 36, animation: "revealUp 0.9s cubic-bezier(0.22,1,0.36,1) 0.25s both" }}>
                  ...
                  {(banner.ctaLabel || banner.ctaLabel2) && (
                    <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap", animation: "revealUp 0.9s cubic-bezier(0.22,1,0.36,1) 0.4s both" }}>
                      {banner.ctaLabel && (banner.ctaLink?.startsWith("#") ? (
                        <button onClick={() => scrollTo(banner.ctaLink!.substring(1))} className="btn-primary float-animation">{isRtl && banner.ctaLabelAr ? banner.ctaLabelAr : banner.ctaLabel} <ArrowIcon rtl={isRtl} /></button>
                      ) : (
                        <Link href={banner.ctaLink || `/${lang}/offers`} className="btn-primary float-animation">{isRtl && banner.ctaLabelAr ? banner.ctaLabelAr : banner.ctaLabel} <ArrowIcon rtl={isRtl} /></Link>
                      ))}
                      {banner.ctaLabel2 && (banner.ctaLink2?.startsWith("#") ? (
                        <button onClick={() => scrollTo(banner.ctaLink2!.substring(1))} className="btn-secondary float-animation-delay" style={{ borderColor: "rgba(255,255,255,0.4)", color: "#fff" }}>{isRtl && banner.ctaLabelAr2 ? banner.ctaLabelAr2 : banner.ctaLabel2}</button>
                      ) : (
                        <Link href={banner.ctaLink2 || `/${lang}/offers`} className="btn-secondary float-animation-delay" style={{ borderColor: "rgba(255,255,255,0.4)", color: "#fff" }}>{isRtl && banner.ctaLabelAr2 ? banner.ctaLabelAr2 : banner.ctaLabel2}</Link>
                      ))}
                    </div>
                  )}
                </div>
            ...
            : (
              <>
                <span className="section-tag" style={{ color: "var(--accent-light)", marginBottom: 20, display: "block", animation: "revealUp 0.9s cubic-bezier(0.22,1,0.36,1) both" }}>{T("heroTag")}</span>
                <h1 style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "clamp(2.8rem,8vw,6.5rem)", fontWeight: isRtl ? 700 : 600, fontStyle: isRtl ? "normal" : "italic", color: "#FFF", marginBottom: 20, lineHeight: 1.02, letterSpacing: "-0.025em", animation: "revealUp 0.9s cubic-bezier(0.22,1,0.36,1) 0.1s both", textShadow: "0 2px 40px rgba(0,0,0,0.4)" }}>{T("heroTitle")}</h1>
                <p style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "clamp(1rem,2.5vw,1.4rem)", fontWeight: 400, fontStyle: isRtl ? "normal" : "italic", color: "rgba(255,255,255,0.80)", marginBottom: 16, animation: "revealUp 0.9s cubic-bezier(0.22,1,0.36,1) 0.25s both" }}>{T("heroSub")}</p>
                <p style={{ fontSize: "clamp(0.88rem,1.5vw,1rem)", color: "rgba(255,255,255,0.65)", marginBottom: 40, maxWidth: 520, marginLeft: "auto", marginRight: "auto", lineHeight: 1.75, animation: "revealUp 0.9s cubic-bezier(0.22,1,0.36,1) 0.35s both" }}>{T("heroDesc")}</p>
                <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap", animation: "revealUp 0.9s cubic-bezier(0.22,1,0.36,1) 0.5s both" }}>
                  ...
                  <button onClick={() => scrollTo("plexi-products")} className="btn-secondary float-animation-delay" style={{ borderColor: "rgba(255,255,255,0.4)", color: "#fff" }}>{T("heroBtnPlexi")}</button>
                </div>
              </>
            )
          }
          ...
          <div style={{ animation: "revealUp 0.9s cubic-bezier(0.22,1,0.36,1) 0.65s both", marginTop: 52, display: "flex", justifyContent: "center" }}>
            <div style={{ display: "inline-flex", flexWrap: "wrap", justifyContent: "center", background: "rgba(255,255,255,0.08)", backdropFilter: "blur(20px) saturate(160%)", WebkitBackdropFilter: "blur(20px) saturate(160%)", border: "1px solid rgba(207,161,141,0.28)", borderRadius: "var(--radius-xl)", padding: "6px 8px", gap: 4, boxShadow: "0 8px 32px rgba(0,0,0,0.18), inset 0 1px 0 rgba(255,255,255,0.12)" }}>
              {heroStats.map((stat, i) => (
                <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", padding: "14px 28px", position: "relative", minWidth: 110 }}>
                  {i > 0 && <span style={{ position: "absolute", left: 0, top: "20%", height: "60%", width: 1, background: "rgba(207,161,141,0.25)" }} />}
                  <span style={{ fontFamily: "var(--font-heading)", fontSize: "clamp(1.6rem,3.5vw,2.2rem)", fontWeight: 700, lineHeight: 1, letterSpacing: "-0.02em", background: "linear-gradient(135deg,#FFF 0%,var(--accent-light) 50%,var(--accent) 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
                    {isRtl && stat.valueAr ? stat.valueAr : stat.value}
                  </span>
                  <span style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-body)", fontSize: "0.68rem", fontWeight: 500, letterSpacing: isRtl ? 0 : "0.14em", textTransform: isRtl ? "none" : "uppercase", color: "rgba(255,255,255,0.60)", marginTop: 6, whiteSpace: "nowrap" }}>
        ...
        {banners.length > 1 && (
          <div style={{ position: "absolute", bottom: 44, left: 0, right: 0, display: "flex", justifyContent: "center", gap: 8, zIndex: 3 }}>
            {banners.map((_, idx) => (
              <button key={idx} onClick={() => setCurrentBannerIndex(idx)} aria-label={`Go to banner ${idx + 1}`} style={{ width: idx === currentBannerIndex ? 28 : 8, height: 8, borderRadius: 4, background: idx === currentBannerIndex ? "var(--accent)" : "rgba(255,255,255,0.35)", border: "none", cursor: "pointer", transition: "all 0.35s ease", padding: 0 }} />
            ))}
          </div>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "center" }} className="brand-story-grid">
          <div className="fade-in-left" style={{ order: isRtl ? 2 : 1 }}>
            ...
            <h2 style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "clamp(2rem,4vw,3rem)", fontWeight: 600, marginBottom: 8, lineHeight: 1.1, letterSpacing: "-0.02em" }}>
              {T("storyTitle1")} <br /><span style={{ fontStyle: isRtl ? "normal" : "italic", color: "var(--accent)" }}>{T("storyTitle2")}</span>
            </h2>
          ...
          <div className="fade-in-right" style={{ position: "relative", order: isRtl ? 1 : 2 }}>
            ...
            <div style={{ position: "absolute", bottom: -24, [isRtl ? "right" : "left"]: -24, zIndex: 2, background: "var(--card-bg)", borderRadius: "var(--radius-md)", padding: "20px 24px", boxShadow: "var(--shadow-hover)", border: "1px solid rgba(207,161,141,0.15)", maxWidth: 220 }}>
              <QuoteIcon />
        ...
        <section style={{ padding: "60px 24px", background: "var(--bg-secondary)" }}>
          <div style={{ maxWidth: 1200, margin: "0 auto" }}>
            <div style={{ textAlign: "center", marginBottom: 40 }}>
              ...
              <h2 style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "clamp(1.6rem,3vw,2.2rem)", fontWeight: 600 }}>{isRtl ? "لا تفوّت هذه الفرصة" : "Don't miss out"}</h2>
            </div>
                ...
                <div key={promo._id} className="fade-in-section luxury-card" style={{ overflow: "hidden", transitionDelay: `${i * 100}ms` }}>
                  {promo.bannerImageUrl && (
                    <div style={{ position: "relative", height: 180, width: "100%" }}>
                      ...
                      <span style={{ position: "absolute", bottom: 16, [isRtl ? "right" : "left"]: 16, background: "var(--gradient-luxury)", color: "#fff", fontSize: "0.72rem", fontWeight: 700, letterSpacing: "0.12em", textTransform: isRtl ? "none" : "uppercase", padding: "5px 12px", borderRadius: "var(--radius-full)" }}>
                  ...
                  <div style={{ padding: "24px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12, gap: 12 }}>
                      <h3 style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "1.15rem", fontWeight: 600, color: "var(--text-primary)" }}>{isRtl && promo.titleAr ? promo.titleAr : promo.title}</h3>
                    ...
                    {(promo.description || promo.descriptionAr) && <p style={{ fontSize: "0.88rem", color: "var(--text-secondary)", marginBottom: 18, lineHeight: 1.7 }}>{isRtl && promo.descriptionAr ? promo.descriptionAr : promo.description}</p>}
                    <Link href={`/${lang}/offers`} className="btn-primary" style={{ fontSize: "0.82rem", padding: "10px 20px" }}>{isRtl ? "اكتشف العرض" : "View Offer"} <ArrowIcon rtl={isRtl} size={14} /></Link>
                  </div>
                </div>
              ))}
            </div>
      ...
      <section id="brands" style={{ padding: "110px 24px", background: "var(--bg-secondary)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 className="fade-in-section" style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "clamp(2rem,4vw,3rem)", fontWeight: 600, marginBottom: 12, letterSpacing: "-0.02em" }}>{T("brandsTitle")}</h2>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 40 }} className="brands-grid">
            <div className="brand-card fade-in-left">
              ...
              <h3 style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "1.7rem", fontWeight: 600, marginBottom: 16 }}>{T("hijabBy")} <span style={{ fontStyle: isRtl ? "normal" : "italic", fontWeight: 400 }}>{T("hijabByTag")}</span></h3>
              <p style={{ color: "var(--text-secondary)", marginBottom: 32, lineHeight: 1.8, fontSize: "0.95rem" }}>{T("hijabDesc")}</p>
              <button onClick={() => scrollTo("hijab-products")} className="btn-primary">{T("hijabBtn")} <ArrowIcon rtl={isRtl} /></button>
            </div>
            <div className="brand-card fade-in-right">
              ...
              <h3 style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "1.7rem", fontWeight: 600, marginBottom: 16 }}>{T("plexiBy")} <span style={{ fontStyle: isRtl ? "normal" : "italic", fontWeight: 400 }}>{T("plexiByTag")}</span></h3>
      ...
      <section id="hijab-products" style={{ padding: "110px 24px", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 28 }} className="product-grid">
          {hijabProducts.length === 0
            ? <p style={{ gridColumn: "1/-1", textAlign: "center", color: "var(--text-secondary)", padding: "80px 0", fontStyle: "italic" }}>{isRtl ? "لا توجد منتجات حجاب متاحة حالياً" : "No hijab products available yet. Check back soon!"}</p>
            : hijabProducts.map((p, i) => {
                const db = dbProducts.find((d) => d.sanityId === p.sanityId);
                const name = isRtl && p.titleAr ? p.titleAr : p.title;
                const oos = db?.stock === 0;
                return (
                  <div key={p._id} className="product-card fade-in-section" style={{ transitionDelay: `${i * 80}ms` }}>
                    <div style={{ position: "relative", aspectRatio: "3/4", overflow: "hidden" }}>
                      <Link href={`/${lang}/products/${p.sanityId}`} aria-label={name} tabIndex={0}>
                        <Image src={p.imageUrl} alt={name} fill style={{ objectFit: "cover" }} sizes="(max-width:768px) 50vw, 25vw" />
                      </Link>
                      {oos && <span className="sold-out-ribbon">{isRtl ? "نفد المخزون" : "Sold Out"}</span>}
                      {!oos && p.isNew && <span className="product-badge">{isRtl ? "جديد" : "New"}</span>}
                      {!oos && p.isBestSeller && !p.isNew && <span className="product-badge">{isRtl ? "الأكثر مبيعاً" : "Bestseller"}</span>}
                      <div style={{ position: "absolute", top: 12, [isRtl ? "left" : "right"]: 12, zIndex: 3 }}><WishlistButton productId={p._id} /></div>
                      <div className="product-card-overlay">
                        <Link href={`/${lang}/products/${p.sanityId}`} className="btn-secondary" style={{ flex: 1, fontSize: "0.78rem", padding: "8px 12px", color: "#fff", borderColor: "rgba(255,255,255,0.5)" }}>{isRtl ? "عرض" : "View"}</Link>
                        {!oos && <button onClick={() => handleAddToCart(p, db)} className="btn-primary" style={{ flex: 1, fontSize: "0.78rem", padding: "8px 12px" }}><CartIcon /> {isRtl ? "أضف" : "Add"}</button>}
                      </div>
                    </div>
                    <div style={{ padding: "20px 24px", textAlign: "center" }}>
                      <Link href={`/${lang}/products/${p.sanityId}`} style={{ textDecoration: "none", color: "inherit" }}>
                        <h3 style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "1.05rem", fontWeight: 500, marginBottom: 4, transition: "color var(--transition-base)" }} onMouseEnter={(e) => ((e.target as HTMLElement).style.color = "var(--accent)")} onMouseLeave={(e) => ((e.target as HTMLElement).style.color = "var(--text-primary)")}>{name}</h3>
                      </Link>
                      <span style={{ fontSize: "0.75rem", color: "var(--accent)", fontWeight: 500, letterSpacing: isRtl ? 0 : "0.08em", display: "block", marginBottom: 14 }}>{T("hijabByLine")}</span>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 14, borderTop: "1px solid rgba(207,161,141,0.12)" }}>
                        <div>
                          {p.discountPrice ? (<><span style={{ fontWeight: 700, color: "var(--text-primary)", fontSize: "1rem" }}>{db ? db.price : p.discountPrice} SYP</span><span style={{ fontSize: "0.8rem", color: "var(--text-secondary)", textDecoration: "line-through", marginInlineStart: 8 }}>{p.price} SYP</span></>) : (<span style={{ fontWeight: 700, color: "var(--text-primary)", fontSize: "1rem" }}>{db ? db.price : p.price} SYP</span>)}
                        </div>
                        <button onClick={() => handleAddToCart(p, db)} className="btn-primary" style={{ padding: "8px 16px", fontSize: "0.78rem" }} disabled={oos}>{isRtl ? "أضف" : "Add"}</button>
                      </div>
                    </div>
                  </div>
                );
        ...
        {hijabProducts.length > 0 && (
          <div style={{ textAlign: "center", marginTop: 56 }}>
            <Link href={`/${lang}/products`} className="btn-secondary float-animation">{isRtl ? "عرض كل منتجات الحجاب" : "View All Hijab Products"} <ArrowIcon rtl={isRtl} /></Link>
          </div>
        )}
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(240px,1fr))", gap: 28 }} className="product-grid">
            {plexiProducts.length === 0
              ? <p style={{ gridColumn: "1/-1", textAlign: "center", color: "var(--text-secondary)", padding: "80px 0", fontStyle: "italic" }}>{isRtl ? "لا توجد منتجات بليكسي متاحة حالياً" : "No plexi products available yet. Check back soon!"}</p>
                  ...
                  return (
                    <div key={p._id} className="product-card plexi-card fade-in-section" style={{ transitionDelay: `${i * 80}ms` }}>
                      <div style={{ position: "relative", aspectRatio: "1", overflow: "hidden" }}>
                        <Link href={`/${lang}/products/${p.sanityId}`} aria-label={name} tabIndex={0}>
                          <Image src={p.imageUrl} alt={name} fill style={{ objectFit: "cover" }} sizes="(max-width:768px) 50vw, 33vw" />
                        </Link>
                        {oos && <span className="sold-out-ribbon">{isRtl ? "نفد المخزون" : "Sold Out"}</span>}
                        {!oos && p.isNew && <span className="product-badge">{isRtl ? "جديد" : "New"}</span>}
                        <div style={{ position: "absolute", top: 12, [isRtl ? "left" : "right"]: 12, zIndex: 3 }}><WishlistButton productId={p._id} /></div>
                      ...
                      <div style={{ padding: "20px 24px", textAlign: "center" }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 14, borderTop: "1px solid rgba(207,161,141,0.12)" }}>
                          <span style={{ fontWeight: 700, color: "var(--text-primary)", fontSize: "1rem" }}>{db ? db.price : p.price} SYP</span>
                          <button onClick={() => handleAddToCart(p, db)} className="btn-secondary" style={{ padding: "8px 16px", fontSize: "0.78rem" }} disabled={oos}>{isRtl ? "أضف" : "Add"}</button>
                        </div>
                      </div>
                    </div>
                  );
          ...
          {plexiProducts.length > 0 && (
            <div style={{ textAlign: "center", marginTop: 56 }}>
              <Link href={`/${lang}/products`} className="btn-secondary float-animation">{isRtl ? "عرض كل منتجات بلكسي" : "View All Plexi Products"} <ArrowIcon rtl={isRtl} /></Link>
            </div>
          )}
        </div>
      </section>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 className="fade-in-section" style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "clamp(2rem,4vw,3rem)", fontWeight: 600, letterSpacing: "-0.02em" }}>
              {isRtl ? "ما يجعلنا " : "What Makes Us "}<span style={{ fontStyle: isRtl ? "normal" : "italic", color: "var(--accent)" }}>{isRtl ? "مميزين" : "Special"}</span>
            </h2>
            <div className="section-divider" />
          </div>
          <div className="reveal-stagger" style={{ maxWidth: 800, margin: "0 auto", position: "relative" }}>
            <div style={{ position: "absolute", top: 40, bottom: 40, [isRtl ? "right" : "left"]: 40, width: 2, background: "linear-gradient(to bottom,rgba(207,161,141,0.1),var(--accent),rgba(207,161,141,0.1))", zIndex: 0 }} className="max-sm:hidden" />
            <div style={{ display: "flex", flexDirection: "column", gap: 16, position: "relative", zIndex: 1 }}>
              {WHY_US_PILLARS.map((pillar, i) => {
                const isActive = activeWhyUs === i;
                return (
                  <div key={i} onClick={() => setActiveWhyUs(isActive ? null : i)} className="luxury-card" style={{ padding: "24px", cursor: "pointer", border: isActive ? "1px solid var(--accent)" : "1px solid rgba(207,161,141,0.15)", transition: "all var(--transition-base)", transform: isActive ? "scale(1.02)" : "scale(1)", boxShadow: isActive ? "0 12px 32px rgba(207,161,141,0.15)" : "var(--shadow-soft)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
                      <div style={{ width: 56, height: 56, borderRadius: "50%", flexShrink: 0, background: isActive ? "var(--accent)" : "rgba(207,161,141,0.1)", color: isActive ? "#fff" : "var(--accent)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.4rem", transition: "all var(--transition-base)" }}>{pillar.icon}</div>
                      <div style={{ flex: 1 }}>
                        <h3 style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "1.25rem", fontWeight: 600, color: "var(--text-primary)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                          {isRtl ? pillar.titleAr : pillar.titleEn}
                          <span style={{ fontSize: "1.5rem", fontWeight: 300, color: "var(--accent)", transform: isActive ? "rotate(45deg)" : "rotate(0deg)", transition: "transform var(--transition-base)" }}>+</span>
                        </h3>
                        <div style={{ maxHeight: isActive ? 200 : 0, opacity: isActive ? 1 : 0, overflow: "hidden", transition: "all 0.4s cubic-bezier(0.22,1,0.36,1)" }}>
                          <p style={{ fontSize: "0.95rem", color: "var(--text-secondary)", lineHeight: 1.7, marginTop: 12 }}>{isRtl ? pillar.descAr : pillar.descEn}</p>
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <section id="custom-orders" style={{ padding: "110px 24px", maxWidth: 900, margin: "0 auto" }}>
        ...
        <div className="fade-in-section luxury-card" style={{ padding: "48px 44px" }}>
          <form onSubmit={handleCustomOrderSubmit} style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }} className="custom-form-grid">
            <div style={{ gridColumn: "1 / -1" }}><label style={{ display: "block", marginBottom: 8, fontSize: "0.83rem", fontWeight: 600, color: "var(--text-secondary)", letterSpacing: "0.04em" }}>{isRtl ? "عنوان الطلب (مثل: خاتم خطوبة مخصص)" : "Request Title (e.g., Custom Engagement Ring)"}</label><input name="title" className="form-input" type="text" placeholder={isRtl ? "عنوان طلبك" : "Title of your request"} required /></div>
            <div><label style={{ display: "block", marginBottom: 8, fontSize: "0.83rem", fontWeight: 600, color: "var(--text-secondary)", letterSpacing: "0.04em" }}>{T("formName")}</label><input name="name" className="form-input" type="text" placeholder={T("formNamePh")} required /></div>
            <div><label style={{ display: "block", marginBottom: 8, fontSize: "0.83rem", fontWeight: 600, color: "var(--text-secondary)", letterSpacing: "0.04em" }}>{T("formEmail")}</label><input name="email" className="form-input" type="email" placeholder={T("formEmailPh")} required /></div>
            <div><label style={{ display: "block", marginBottom: 8, fontSize: "0.83rem", fontWeight: 600, color: "var(--text-secondary)", letterSpacing: "0.04em" }}>{T("formColor")}</label><input name="color" className="form-input" type="text" placeholder={T("formColorPh")} /></div>
            <div><label style={{ display: "block", marginBottom: 8, fontSize: "0.83rem", fontWeight: 600, color: "var(--text-secondary)", letterSpacing: "0.04em" }}>{T("formOccasion")}</label><input name="occasion" className="form-input" type="text" placeholder={T("formOccasionPh")} /></div>
            <div><label style={{ display: "block", marginBottom: 8, fontSize: "0.83rem", fontWeight: 600, color: "var(--text-secondary)", letterSpacing: "0.04em" }}>{isRtl ? "الكمية المطلوبة" : "Requested Quantity"}</label><input name="requestedQuantity" className="form-input" type="number" min="1" defaultValue="1" required /></div>
            <div style={{ gridColumn: "1 / -1" }}><label style={{ display: "block", marginBottom: 8, fontSize: "0.83rem", fontWeight: 600, color: "var(--text-secondary)", letterSpacing: "0.04em" }}>{T("formMsg")}</label><textarea name="message" className="form-input" rows={4} placeholder={T("formMsgPh")} style={{ resize: "vertical" }} required /></div>
            <div style={{ gridColumn: "1 / -1" }}>
              <label style={{ display: "block", marginBottom: 8, fontSize: "0.83rem", fontWeight: 600, color: "var(--text-secondary)", letterSpacing: "0.04em" }}>
                {isRtl ? "إرفاق صور (اختياري، يمكنك اختيار أكثر من صورة)" : "Attach Images (Optional, select multiple)"}
              </label>
              <input name="images" type="file" multiple accept="image/*" className="form-input" style={{ padding: "12px", background: "rgba(207,161,141,0.03)" }} />
            </div>
            <div style={{ gridColumn: "1 / -1", textAlign: "center", marginTop: 8 }}>
              <button type="submit" className="btn-primary" style={{ padding: "16px 52px", fontSize: "1rem", opacity: customOrderLoading ? 0.75 : 1, cursor: customOrderLoading ? "not-allowed" : "pointer" }} disabled={customOrderLoading}>
                {customOrderLoading ? (<span style={{ display: "flex", alignItems: "center", gap: 10 }}><span style={{ width: 18, height: 18, border: "2px solid rgba(255,255,255,0.35)", borderTopColor: "white", borderRadius: "50%", display: "inline-block", animation: "spin 0.75s linear infinite" }} />{isRtl ? "جارٍ الإرسال…" : "Sending…"}</span>) : <>{T("formBtn")} <ArrowIcon rtl={isRtl} /></>}
              </button>
            </div>
          </form>
        </div>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          <div className="fade-in-section" style={{ position: "relative", minHeight: 280 }}>
            {testimonialData.map((tm, i) => (
              <div key={i} className="testimonial-card" style={{ position: i === activeTestimonial ? "relative" : "absolute", top: 0, left: 0, right: 0, opacity: i === activeTestimonial ? 1 : 0, transform: i === activeTestimonial ? "translateY(0)" : "translateY(20px)", transition: "all 0.7s cubic-bezier(0.22,1,0.36,1)", pointerEvents: i === activeTestimonial ? "auto" : "none", textAlign: "center" }}>
                {tm.avatarUrl ? (
                  <div style={{ width: 64, height: 64, borderRadius: "50%", overflow: "hidden", margin: "0 auto 20px", border: "3px solid var(--accent)", boxShadow: "var(--shadow-glow-sm)" }}><Image src={tm.avatarUrl} alt={tm.author} width={64} height={64} style={{ objectFit: "cover", width: "100%", height: "100%" }} /></div>
                ) : (
                  <div style={{ margin: "0 auto 20px" }}><QuoteIcon /></div>
                )}
                <div style={{ display: "flex", gap: 4, justifyContent: "center", marginBottom: 20 }}>{[...Array(5)].map((_, j) => <StarIcon key={j} filled={j < (tm.rating || 5)} />)}</div>
                <div style={{ display: "flex", justifyContent: "center", marginBottom: 20 }}>
                  <span style={{ display: "flex", alignItems: "center", gap: 4, background: "rgba(207,161,141,0.1)", color: "var(--accent)", padding: "3px 10px", borderRadius: "var(--radius-full)", fontSize: "0.68rem", fontWeight: 700 }}><CheckIcon /> {isRtl ? "مشتري موثق" : "Verified Purchase"}</span>
                </div>
                <p style={{ fontFamily: isRtl ? "var(--font-arabic)" : "var(--font-heading)", fontSize: "clamp(1rem,2vw,1.2rem)", fontStyle: isRtl ? "normal" : "italic", lineHeight: 1.75, color: "var(--text-primary)", marginBottom: 24, maxWidth: 580, marginLeft: "auto", marginRight: "auto" }}>&ldquo;{tm.text}&rdquo;</p>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 40 }}>
              {testimonialData.map((_, i) => (
                <button key={i} onClick={() => setActiveTestimonial(i)} aria-label={`Show testimonial ${i + 1}`} aria-current={i === activeTestimonial ? "true" : undefined} style={{ width: i === activeTestimonial ? 32 : 10, height: 10, borderRadius: "var(--radius-full)", background: i === activeTestimonial ? "var(--gradient-luxury)" : "var(--highlight)", border: "none", cursor: "pointer", transition: "all 0.3s ease", padding: 0 }} />
              ))}
            </div>
          </div>
        </div>
          ...
          <div className="fade-in-left luxury-card" style={{ padding: "40px 36px" }}>
            ...
            <form onSubmit={handleContactSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              ...
              <button type="submit" className="btn-primary" style={{ alignSelf: isRtl ? "flex-end" : "flex-start", opacity: contactLoading ? 0.75 : 1, cursor: contactLoading ? "not-allowed" : "pointer" }} disabled={contactLoading}>
                {contactLoading ? (<span style={{ display: "flex", alignItems: "center", gap: 8 }}><span style={{ width: 16, height: 16, border: "2px solid rgba(255,255,255,0.35)", borderTopColor: "white", borderRadius: "50%", display: "inline-block", animation: "spin 0.75s linear infinite" }} />{isRtl ? "جارٍ الإرسال…" : "Sending…"}</span>) : T("contactSendBtn")}
              </button>
            </form>
          </div>
      ...
      {/* TOAST */}
      {toast && (
        <div style={{ position: "fixed", bottom: 32, [isRtl ? "left" : "right"]: 32, zIndex: 9999, display: "flex", alignItems: "center", gap: 12, background: toast.type === "success" ? "linear-gradient(135deg,#3A2E2A 0%,#6B5B55 100%)" : "linear-gradient(135deg,#7f1d1d 0%,#991b1b 100%)", color: "white", padding: "16px 20px", borderRadius: 18, boxShadow: "0 24px 64px rgba(0,0,0,0.35),0 0 0 1px rgba(255,255,255,0.05)", animation: "slideInToast 0.4s cubic-bezier(0.34,1.56,0.64,1)", maxWidth: 380, fontSize: "0.9rem", lineHeight: 1.5 }}>
